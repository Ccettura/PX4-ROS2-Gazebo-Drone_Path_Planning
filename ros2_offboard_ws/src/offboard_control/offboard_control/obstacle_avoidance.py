...
class OffboardNav2Bridge(Node):
    ...
    - LAND a goal completato + disarmo quando PX4 rileva 'landed'
    """

    PX4_NS = "/chotto/fmu"
    TAKEOFF_ALTITUDE = 0.3          # m AGL (NED target z = -0.3)
    ...
    def __init__(self):
        ...
        # NEW: gestione landing/disarmo
        self.landing = False      # stiamo eseguendo un LAND?
        self.landed = False       # PX4 segnala 'landed'?
        self.disarmed = False     # abbiamo già disarmato?
    ...
    def cb_odom(self, msg: VehicleOdometry):
        ...
        if not self.takeoff_done:
            # target è negativo (−0.3): consideriamo raggiunto quando z <= target + eps
    ...
    def _land_and_disarm(self):
        # NIENTE timer periodico: disarmiamo solo quando 'landed' diventa True
        self.landing = True
        self.get_logger().info("LAND comandato. Attendo stato 'landed' per disarmare…")
    ...
    def _goal_result_cb(self, future):
        ...
        # Al completamento: comanda LAND e disarma SOLO quando 'landed'
        self._land_and_disarm()

    # === Streaming setpoint ===
    def stream_offboard(self):
        # Durante il LAND lasciamo lavorare PX4; evitiamo conflitti con AUTO.LAND
        if self.landing:
            return
